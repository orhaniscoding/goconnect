...
func TestSqliteAuditor_Hashing(t *testing.T) {
    ...
    if len(evs) != 1 { t.Fatalf("expected 1 event got %d", len(evs)) }
    ...
    if len(evs2) != 1 { t.Fatalf("expected 1 after reopen got %d", len(evs2)) }
}
...
func BenchmarkSqliteAuditor_Event(b *testing.B) {
    ...
    a, err := NewSqliteAuditor(dbPath)
    if err != nil { b.Fatalf("new auditor: %v", err) }
    defer a.Close()
    ctx := context.Background()
    b.ResetTimer()
    for i := 0; i < b.N; i++ {
    ...
    if _, err := os.Stat(dbPath); err != nil { b.Fatalf("db not created: %v", err) }
}
