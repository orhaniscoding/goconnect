...
func WithMaxAge(d time.Duration) SqliteOption { return func(a *SqliteAuditor) { if d > 0 { a.maxAge = d } } }
func WithMaxRows(n int) SqliteOption         { return func(a *SqliteAuditor) { if n > 0 { a.maxRows = n } } }
func WithAnchorInterval(n int) SqliteOption  { return func(a *SqliteAuditor) { if n > 0 { a.anchorInterval = n } } }
...
func WithSqliteHashSecrets(secrets ...[]byte) SqliteOption {
    return func(a *SqliteAuditor) { h, _, _ := multiSecretHasher(secrets); if h != nil { a.hasher = h } }
}
func WithIntegritySigningKey(priv ed25519.PrivateKey) SqliteOption {
    return func(a *SqliteAuditor) { if len(priv) == ed25519.PrivateKeySize { a.signingKey = priv } }
}
func WithIntegritySigningKeyID(kid string, priv ed25519.PrivateKey) SqliteOption {
    return func(a *SqliteAuditor) { if len(priv) == ed25519.PrivateKeySize { a.signingKey = priv; a.signingKeyID = kid } }
}
...
func NewSqliteAuditor(dsn string, opts ...SqliteOption) (*SqliteAuditor, error) {
    ...
    if err := a.migrate(); err != nil { return nil, err }
    return a, nil
}
...
func (a *SqliteAuditor) migrate() error {
    ...
    if _, err := a.db.Exec(schema); err != nil { return fmt.Errorf("migrate audit_events: %w", err) }
...
func (a *SqliteAuditor) Event(ctx context.Context, action, actor, object string, details map[string]any) {
    ...
    if a.hasher != nil { actOut = a.hasher(actor); objOut = a.hasher(object) }
    ...
    if _, err := a.db.ExecContext(ctx, `INSERT INTO audit_events(ts, action, actor, object, details, request_id, chain_hash) VALUES(?,?,?,?,?,?,?)`, ts, action, actOut, objOut, string(b), rid, chainHash); err != nil {
        metrics.IncAuditFailure("exec"); status = "failure"
    }
    ...
    if a.maxRows > 0 {
        ...
        if err == nil { if rows, _ := res.RowsAffected(); rows > 0 { metrics.AddAuditEviction("sqlite", int(rows)) } } else { metrics.IncAuditFailure("prune") }
    }
    ...
    if a.maxAge > 0 {
        ...
        if err == nil { if rows, _ := res.RowsAffected(); rows > 0 { metrics.AddAuditEviction("sqlite", int(rows)) } } else { metrics.IncAuditFailure("prune_age") }
        if _, err := a.db.ExecContext(ctx, `DELETE FROM audit_chain_anchors WHERE seq NOT IN (SELECT seq FROM audit_events)`); err != nil { metrics.IncAuditFailure("prune_anchor") }
    }
    ...
    if status == "success" && a.anchorInterval > 0 {
        ...
        if err := row.Scan(&seq); err == nil {
            if seq%int64(a.anchorInterval) == 0 {
                if _, err := a.db.ExecContext(ctx, `INSERT OR IGNORE INTO audit_chain_anchors(seq, ts, chain_hash) VALUES(?,?,?)`, seq, ts, chainHash); err != nil { metrics.IncAuditFailure("anchor_insert") } else { metrics.IncChainAnchor() }
            }
        }
    }
}
...
func (a *SqliteAuditor) ListRecent(ctx context.Context, limit int) ([]EventRecord, error) {
    ...
    for rows.Next() {
        ...
        if err := rows.Scan(&tsStr, &action, &actor, &object, &detailsJSON, &rid); err != nil { return nil, err }
        t, _ := time.Parse(time.RFC3339Nano, tsStr)
        det := map[string]any{}
        if detailsJSON.Valid && detailsJSON.String != "" { _ = json.Unmarshal([]byte(detailsJSON.String), &det) }
...
func (a *SqliteAuditor) ExportIntegrity(ctx context.Context, limit int) (IntegrityExport, error) {
    ...
    if len(a.signingKey) == ed25519.PrivateKeySize {
        ...
        tmpExp := exp; tmpExp.Signature = ""
        payload, err := json.Marshal(tmpExp)
        if err == nil { sig := ed25519.Sign(a.signingKey, payload); exp.Signature = base64.RawURLEncoding.EncodeToString(sig); metrics.IncIntegritySigned() } else { metrics.IncAuditFailure("integrity_sign") }
    }
    return exp, nil
}
...
func (a *SqliteAuditor) VerifyChain(ctx context.Context) error {
    ...
    for rows.Next() {
        ...
        if index == 0 && prev == "" {
            if !storedHash.Valid { metrics.ObserveChainVerification(time.Since(start).Seconds(), false); return fmt.Errorf("chain baseline missing hash at first retained row") }
            prev = storedHash.String
        } else {
            if !storedHash.Valid || storedHash.String != recomputed { metrics.ObserveChainVerification(time.Since(start).Seconds(), false); return fmt.Errorf("chain mismatch at index %d hash=%s expected=%s", index, storedHash.String, recomputed) }
            prev = recomputed
        }
        index++
    }
...
func (a *SqliteAuditor) ListAnchors(ctx context.Context) ([]struct{ Seq int64; TS, Hash string }, error) {
    rows, err := a.db.QueryContext(ctx, `SELECT seq, ts, chain_hash FROM audit_chain_anchors ORDER BY seq ASC`)
    if err != nil { return nil, err }
    defer rows.Close()
    var out []struct{ Seq int64; TS, Hash string }
    for rows.Next() {
        var seq int64; var ts, h string
        if err := rows.Scan(&seq, &ts, &h); err != nil { return nil, err }
        out = append(out, struct{ Seq int64; TS, Hash string }{Seq: seq, TS: ts, Hash: h})
    }
    return out, nil
}
...
func (a *SqliteAuditor) VerifyFromAnchor(ctx context.Context, anchorSeq int64) error {
    if anchorSeq <= 0 { return a.VerifyChain(ctx) }
    start := time.Now()
    var prev string
    if anchorSeq > 1 { row := a.db.QueryRowContext(ctx, `SELECT chain_hash FROM audit_events WHERE seq = ?`, anchorSeq-1); _ = row.Scan(&prev) }
    ...
    for rows.Next() {
        var seq int64
        var ts, action, actor, object, detailsJSON, rid, storedHash sql.NullString
        if err := rows.Scan(&seq, &ts, &action, &actor, &object, &detailsJSON, &rid, &storedHash); err != nil { return err }
        detMap := map[string]any{}
        if detailsJSON.Valid && detailsJSON.String != "" { _ = json.Unmarshal([]byte(detailsJSON.String), &detMap) }
        canonicalDetails := canonicalizeDetails(detMap)
        chainInput := prev + "|" + ts.String + "|" + action.String + "|" + actor.String + "|" + object.String + "|" + canonicalDetails + "|" + rid.String
        recomputed := sha256Hex(chainInput)
        if index == 0 && prev == "" {
            if !storedHash.Valid { metrics.ObserveChainVerification(time.Since(start).Seconds(), false); return fmt.Errorf("partial chain baseline missing hash at first retained row seq %d", seq) }
            prev = storedHash.String
        } else {
            if !storedHash.Valid || storedHash.String != recomputed { metrics.ObserveChainVerification(time.Since(start).Seconds(), false); return fmt.Errorf("partial chain mismatch at index %d seq %d hash=%s expected=%s", index, seq, storedHash.String, recomputed) }
            prev = recomputed
        }
        index++
    }
