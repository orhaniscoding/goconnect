...
func TestAnchorCreationAndPartialVerify(t *testing.T) {
    ...
    if err != nil { t.Fatalf("list anchors: %v", err) }
    if len(anchors) != 2 { t.Fatalf("expected 2 anchors got %d", len(anchors)) }
    if anchors[0].Seq != 2 || anchors[1].Seq != 4 { t.Fatalf("unexpected anchor seqs: %+v", anchors) }
...
func TestPartialVerifyMismatchAfterAnchor(t *testing.T) {
    ...
    if err != nil { t.Fatalf("new sqlite auditor: %v", err) }
    ctx := context.Background()
    aud.Event(ctx, "A1", "actor", "obj", nil) // seq1 anchor
    aud.Event(ctx, "A2", "actor", "obj", nil) // seq2 anchor
