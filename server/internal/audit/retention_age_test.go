...
func TestAgeRetentionPrunesOldEvents(t *testing.T) {
    ...
    if c != 2 { t.Fatalf("expected 2 events retained got %d", c) }
    if err := aud.VerifyChain(ctx); err != nil { t.Fatalf("verify after prune: %v", err) }
}
...
func TestAgeRetentionTamperDetected(t *testing.T) {
    ...
    if err != nil { t.Fatalf("tamper: %v", err) }
    if err := aud.VerifyChain(ctx); err == nil { t.Fatalf("expected verification failure after tamper") }
}