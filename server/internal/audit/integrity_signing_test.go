...
func TestIntegrityExportSigningWithKeyID(t *testing.T) {
	...
	if exp.KeyID != kid { t.Fatalf("expected kid %s got %s", kid, exp.KeyID) }
	...
	sigBytes, err := base64.RawURLEncoding.DecodeString(exp.Signature)
	if err != nil { t.Fatalf("decode sig: %v", err) }
	if !ed25519.Verify(pub, payload, sigBytes) { t.Fatalf("signature verify failed with kid payload") }
}
