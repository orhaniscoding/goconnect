...
func TestIPAllocationReleaseAudit(t *testing.T) {
    ...
    if err := nrepo.Create(context.Background(), netw); err != nil { t.Fatalf("create network: %v", err) }
    ...
    r.ServeHTTP(w, req)
    if w.Code != http.StatusOK { t.Fatalf("alloc expected 200 got %d body=%s", w.Code, w.Body.String()) }
    ...
    if !foundAlloc || !foundRelease { t.Fatalf("expected both IP_ALLOCATED and IP_RELEASED events; got=%v", store.List()) }
}
...
func TestAdminReleaseAudit(t *testing.T) {
    ...
    if _, err := ips.AllocateIP(context.Background(), netw.ID, "memberB"); err != nil { t.Fatalf("alloc target: %v", err) }
    ...
    req.Header.Set("Idempotency-Key", domain.GenerateIdempotencyKey())
    r.ServeHTTP(w, req)
    if w.Code != http.StatusNoContent { t.Fatalf("admin release expected 204 got %d body=%s", w.Code, w.Body.String()) }
    // find release event with released_for
    found := false
    for _, ev := range store.List() {
        if ev.Action == "IP_RELEASED" {
            if rf, ok := ev.Details["released_for"]; ok && rf == "memberB" { found = true; break }
        }
    }
    if !found { t.Fatalf("expected IP_RELEASED event with released_for detail; events=%v", store.List()) }
}
