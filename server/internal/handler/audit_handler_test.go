...
func TestAuditIntegrityHandler(t *testing.T) {
    ...
    for i:=0;i<5;i++ { aud.Event(ctx, "ACT", "actor", "obj", map[string]any{"i":i}) }
    r := gin.New()
    ...
    var resp struct {
        Head struct { Seq int64 `json:"seq"`; Hash string `json:"hash"`; TS string `json:"ts"` } `json:"head"`
        Anchors []struct { Seq int64 `json:"seq"`; Hash string `json:"hash"`; TS string `json:"ts"` } `json:"anchors"`
        LatestSeq int64 `json:"latest_seq"`
    }
    if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil { t.Fatalf("unmarshal: %v", err) }
    if resp.Head.Seq == 0 || resp.Head.Hash == "" { t.Fatalf("missing head in response: %+v", resp) }
    if resp.LatestSeq != resp.Head.Seq { t.Fatalf("latest_seq mismatch head.seq") }
    if len(resp.Anchors) == 0 { t.Fatalf("expected anchors present") }
