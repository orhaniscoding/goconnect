...
func runSetupWizard(reader *bufio.Reader, httpClient *http.Client, saveConfig func(*config.Config, string) error) {
	...
	if installService == "" || installService == "y" || installService == "yes" {
		...
		// Use the implementation from service manager or legacy RunDaemon?
		// For now, print instructions as previous.
		// Or better, call runServiceAction("install") if we can.
		// But that requires manager.
		// Retaining old behavior for now.
		fmt.Println("  ⚠️  Note: This may require administrator/root privileges.")
		fmt.Println()
		fmt.Println("  To install the service manually, run:")
		fmt.Println("    goconnect service install")
		fmt.Println()
		fmt.Println("  After installing, start it with:")
		fmt.Println("    goconnect service start")
		fmt.Println()
	...
	fmt.Println("  ─────────────────────────────────────────────────────────────")
	fmt.Println("  1. Install the service:  goconnect service install")
	fmt.Println("  2. Start the service:    goconnect service start")
