...
- [Server Configuration (YAML)](#server-configuration-yaml)
...
GoConnect loads configuration in the following order of priority (highest to lowest):

1.  **Command Line Flags** (e.g., `--port=9090`)
2.  **Environment Variables** (e.g., `SERVER_PORT=9090`)
3.  **Configuration File** (`goconnect.yaml`)
4.  **Defaults**

## Server Configuration (YAML)

The server uses a `goconnect.yaml` file for persistent configuration. This file is automatically generated when you run the **Setup Wizard**.

**Default Locations:**
- **Linux/macOS:** `./goconnect.yaml` or `/etc/goconnect/goconnect.yaml`
- **Windows:** `./goconnect.yaml` or `C:\ProgramData\GoConnect\goconnect.yaml`

### Example `goconnect.yaml`
...
database:
  ...
  # Postgres settings (ignored if backend is sqlite)
  host: "localhost"
  port: "5432"
  name: "goconnect"
  user: "goconnect"
...
jwt:
  secret: "long_secure_random_string_base64"
  access_ttl: 15m
  refresh_ttl: 168h
...
wireguard:
  ...
  server_pubkey: "base64_public_key"
  interface_name: "wg0"
  port: 51820

redis:
  host: "" # Optional, for external caching
```
...
Environment variables can be used to override settings in `goconnect.yaml` or to configure the server in containerized environments (Docker).

### Configuration File Location

The server looks for `.env` files in:
- **All platforms**: `/etc/goconnect/.env` or `.env` in the working directory.
