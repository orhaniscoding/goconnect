...
func TestKeyRotationDualSecrets(t *testing.T) {
	...
	if oldHash == newHash {
		t.Fatalf("expected different hashes after rotation, got same %s", oldHash)
	}

	// Validate deterministic hashing for old secret still matches stored historical event
	singleHasher := newHasher(secretA)
	if singleHasher("actor-1") != oldHash {
		t.Fatalf("rehash with old secret mismatch")
	}
	if newHasher(secretB)("actor-1") != newHash {
		t.Fatalf("rehash with new secret mismatch")
	}
	...
	if historicalHash != legacyHash {
		t.Fatalf("historical hash changed; expected %s got %s", legacyHash, historicalHash)
	}
	if newestHash == legacyHash {
		t.Fatalf("expected new hash after rotation, still %s", newestHash)
	}

	// Re-hash validations
	if newHasher(secretA)("actor-1") != legacyHash {
		t.Fatalf("old secret rehash mismatch sqlite")
	}
	if newHasher(secretB)("actor-1") != newestHash {
		t.Fatalf("new secret rehash mismatch sqlite")
	}
